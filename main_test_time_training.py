...
def get_args_parser():
    ...
    # Online TTT
    parser.add_argument('--online_ttt', default=0, type=int, help='Online TTT')
    parser.add_argument('--change_classes', default=0, type=int, help='Discard model when changing to the next class')
    parser.add_argument('--shuffle_classes', default=0, type=int, help='Shuffle the images')
    parser.add_argument('--several_severities', default=0, type=int, help='Several severities (1 to 5)')
    parser.add_argument('--shuffle_severities', default=0, type=int, help='Shuffle severities')
...
def main(args):
    ...
    if args.shuffle_classes:
        num_images = len(glob.glob(os.path.join(data_path, '*', '*.JPEG')))
        print(f'Shuffling classes, number of images: {num_images}')
        minimizer = np.random.permutation(num_images)
    else:
        minimizer = None

    if not args.several_severities:
        dataset_train = tt_image_folder.ExtendedImageFolder(data_path, transform=transform_train, minimizer=minimizer, 
        ...
        dataset_val = tt_image_folder.ExtendedImageFolder(data_path, transform=transform_val, 
                                                            batch_size=1, minimizer=minimizer, 
                                                            single_crop=args.single_crop, start_index=max_known_file+1)
    else:
        dataset_train = tt_image_folder.severalNoises(data_path, batch_size=args.batch_size,
                                                      steps_per_example=args.steps_per_example * args.accum_iter,
                                                      transform=transform_train,
                                                      single_crop=args.single_crop,
                                                      shuffle=args.shuffle_severities)
        dataset_val = tt_image_folder.severalNoises(data_path, batch_size=1, transform=transform_val,
                                                    single_crop=args.single_crop,
                                                    shuffle = args.shuffle_severities,
                                                    order = dataset_train.order_ if args.shuffle_severities else None)
        # assert False
        if args.shuffle_severities:
            print(f"Shuffling severities")
            np.save(os.path.join(args.output_dir, 'order_.npy'), dataset_train.order_)
            np.save(os.path.join(args.output_dir, 'order.npy'), dataset_train.order)
