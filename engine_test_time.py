...
def train_on_test(base_model: torch.nn.Module,
    ...
    previous_test_label = None

    for data_iter_step in tqdm(range(iter_start, dataset_len), desc='Data Iteration', total=dataset_len):
        val_data = next(val_loader)
        (test_samples, test_label) = val_data

        if (not args.online_ttt) or (args.change_classes and args.online_ttt) and (previous_test_label is not None and test_label[0].cpu().detach().numpy() != previous_test_label):
            model, optimizer, loss_scaler = _reinitialize_model(base_model, base_optimizer, base_scalar, clone_model, args, device)
            print("Reinitialized model")
        previous_test_label = test_label[0].cpu().detach().numpy()
        ...
        for step_per_example in range(args.steps_per_example * accum_iter):
            ...
            if (step_per_example + 1) % accum_iter == 0:
                with torch.no_grad():
                    ...
                    model.train()
        with open(os.path.join(args.output_dir, f'log_output.txt'), 'a') as f:
            f.write(f'step: {data_iter_step}, acc {np.mean(all_results[-1])}, rec-loss {loss_value}\n')
        if data_iter_step % 200 == 199 or (data_iter_step == dataset_len - 1):
